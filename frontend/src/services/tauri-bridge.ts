...
function resolveInvokeBridge(): TauriInvoke | null {
  ...
  console.log('[tauri-bridge] Checking for Tauri bridge...', {
    hasGlobalTauri: !!tauriWindow.__TAURI__,
    hasCore: !!tauriWindow.__TAURI__?.core,
    hasInvoke: !!tauriWindow.__TAURI__?.core?.invoke,
    hasInternals: !!tauriWindow.__TAURI_INTERNALS__,
    keys: Object.keys(window).filter(k => k.includes('TAURI'))
  });
