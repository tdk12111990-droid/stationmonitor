...
#[tauri::command]
fn stop_yolo_inference(state: tauri::State<'_, YoloState>) -> Result<(), String> {
    let mut slot = state.child.lock().map_err(|_| "Failed to lock YOLO state")?;
    if let Some(mut child) = slot.take() {
        let _ = child.kill();
    }
    Ok(())
...
#[tauri::command]
async fn run_yolo_inference(source: String, app: AppHandle, state: tauri::State<'_, YoloState>) -> Result<String, String> {
    let script_path = if cfg!(debug_assertions) {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().unwrap().join("worldmonitor-main/scripts/yolo_test.py")
    } else {
        app.path().resource_dir().unwrap().join("scripts/yolo_test.py")
    };

    if !script_path.exists() {
        return Err(format!("YOLO script missing at {}", script_path.display()));
    }
    ...
    let mut child = cmd.spawn().map_err(|e| format!("Failed to start YOLO: {e}"))?;
    
    // For a simple "test", we wait for the first line of JSON output
    // In a real implementation, we'd use a channel or sidecar events
    use std::io::{BufRead, BufReader};
    let stdout = child.stdout.take().ok_or("Failed to capture stdout")?;
    let mut reader = BufReader::new(stdout);
    let mut line = String::new();
    
    // We store the child to kill it later if needed, but here we wait for result
    // To make it "live", we would need a different architecture, but this is a "test" as requested.
