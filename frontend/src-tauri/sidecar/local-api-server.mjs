...
async function dispatch(requestUrl, req, routes, context) {
  ...
  if (requestUrl.pathname === '/api/capture' && req.method === 'POST') {
    try {
      const body = await readBody(req);
      const { streamId = 'hikvision_main' } = body ? JSON.parse(body.toString()) : {};
      const snapshotUrl = `http://127.0.0.1:1984/api/frame.jpeg?src=${streamId}`;
      
      const response = await fetch(snapshotUrl);
      if (!response.ok) throw new Error('Failed to fetch snapshot from go2rtc');
      const buffer = Buffer.from(await response.arrayBuffer());
      
      const captureDir = path.join(context.resourceDir, 'captures');
      ...
      return json({ success: true, filename, path: fullPath });
    } catch (error) {
      return json({ success: false, error: error.message }, 500);
    }
  }

  // Serve captured images (Hardened with Absolute Path & Wide CORS)
  if (requestUrl.pathname.startsWith('/api/captures/')) {
    const filename = requestUrl.pathname.replace('/api/captures/', '');
    if (!filename) return json({ error: 'Filename missing' }, 400);

    // Using explicit absolute path found via walk
    const capturesDir = 'D:\\Studies\\Research\\DETAITHAYGIAO\\worldmonitor-main\\worldmonitor-main\\captures';
    const fullPath = path.join(capturesDir, filename);
    
    try {
      const { existsSync, lstatSync, readFileSync } = await import('node:fs');
      ...
      console.error(`[local-api] Serve error: ${err.message}`);
    }

    console.error(`[local-api] Image not found or is directory: ${fullPath}`);
    return json({ error: 'Image not found', path: fullPath }, 404);
  }



  // Get list of recent captures
  if (requestUrl.pathname === '/api/captures-list') {
    const capturesDir = 'D:\\Studies\\Research\\DETAITHAYGIAO\\worldmonitor-main\\worldmonitor-main\\captures';
    if (!existsSync(capturesDir)) {
      return json({ captures: [] }, 200, { 'access-control-allow-origin': '*' });
    }
    ...
    const files = readdirSync(capturesDir)
      ...
      .map(f => ({
        name: f,
        time: statSync(path.join(capturesDir, f)).mtimeMs
      }))
      .sort((a, b) => b.time - a.time)
      .slice(0, 10);
    
    return json({ captures: files.map(f => f.name) }, 200, { 'access-control-allow-origin': '*' });
  }
...
export async function createLocalApiServer(options = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (!headers['access-control-allow-origin'] || headers['access-control-allow-origin'] === 'tauri://localhost') {
